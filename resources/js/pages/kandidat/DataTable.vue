...
const getFilterDisplayValue = (value: any, columnId?: string): string => {
    ...
    // Handle kegiatan display
    if (columnId === 'id_kegiatan') {
        const kegiatanName = props.helpers?.kegiatan?.find(k => k.id === Number(value))?.nama
        return kegiatanName || String(value)
    }
    ...
    <div class="flex flex-col w-full gap-4">
        ...
        <div class="flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
            <div class="space-y-1 mb-2 md:mb-4">
                <h2 class="text-xl md:text-2xl font-bold tracking-tight">Data Kandidat</h2>
                <p class="text-muted-foreground">Kelola data kandidat Pemira FMIPA</p>
            </div>

            <div class="flex items-center justify-center max-w-lg gap-4">
                <!-- Search bar -->
                <div class="flex justify-end items-center w-full">
                    <Input placeholder="Cari kandidat..." :model-value="globalFilter"
                    ...
                    <DropdownMenuContent align="end" class="w-[200px]">
                        <div class="flex flex-col gap-3 p-2">
                            ...
                            <div class="flex flex-col gap-2">
                                <Select :model-value="String(table.getColumn('id_kegiatan')?.getFilterValue())"
                                    @update:model-value="(value) => table.getColumn('id_kegiatan')?.setFilterValue(value === 'all' ? undefined : value)">
                                    ...
                                    <SelectContent>
                                        ...
                                        <SelectItem v-for="value in getUniqueValues('id_kegiatan')" :key="value"
                                            :value="value" class="capitalize text-foreground">
                                            {{props.helpers?.kegiatan?.find(k => k.id === Number(value))?.nama ||
                                                value}}
                                        </SelectItem>
                                    </SelectContent>
                                </Select>
                ...
                <Dialog v-model:open="isDialogOpen">
                    ...
                    <Form mode="create" :mahasiswa="helpers!.mahasiswa" :kegiatan="helpers!.kegiatan" :kandidat="helpers!.kandidat" @success="handleSuccess" />
                </Dialog>
            </div>
        </div>
        ...
        <Dialog v-model:open="isViewDialogOpen">
            <Form v-if="selectedRowData" mode="view" :kandidat-data="selectedRowData" :mahasiswa="helpers!.mahasiswa"
                :kegiatan="helpers!.kegiatan" :kandidat="helpers!.kandidat" @success="isViewDialogOpen = false" />
        </Dialog>
    </div>
</template>